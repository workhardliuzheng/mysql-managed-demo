...
| 参数项               | 说明                                                            |
|-------------------|---------------------------------------------------------------|
| 数据库root和admin账户密码 | 数据库root和admin账户密码，长度8-32个字符，可包含大小字母、数字及特殊符号（包含：!@#$%^&*-+=_）。 |
...
5. 使用服务，本服务支持私网访问，租户可以通过PrivateLink实现私网访问（[私网访问帮助文档](https://bp.aliyun.com/detail/186?spm=5176.9843921.content.344.4a604882OYKl3N)），
   租户私网访问流程：
   1. 创建终端节点，这里选择的终端节点服务为上图中的EndpointServiceId，还需要选择租户自己的VPC和可用区，后面在使用时可以在该可用区下创建ECS，实现在该VPC的ECS访问到服务商VPC内的MySQL
   ![image.png](创建终端节点.jpg)
   2. 成功创建终端节点后可以看到终端节点的状态为"已连接"
   ![image.png](成功创建终端节点.jpg)
   3. 租户可以在"终端节点"处看到对应的访问终端节点服务的链接或地址,这里使用可用区域名或者IP地址都能进行访问
   ![image.png](privateLink访问地址.jpg)
   4. 租户在对应的可用区下创建ECS，并连接MySQL，下面展示分别用IP与可用区域名进行连接
   ![image.png](访问数据库1.jpg)
   ![image.png](访问数据库2.jpg)
